...
gg <- ggplot(data = dt, aes(Var2, Var1, fill = value)) + geom_raster() + viridis::scale_fill_viridis(na.value = NA) + geom_text(aes(fill = dt$value, label = dt$value), size = 10) + coord_flip() + theme_minimal() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), legend.position = "none", axis.text = element_text(size = 20), panel.grid = element_blank()) + ggtitle(expression(paste("Correlation Matrix (",rho, ")")))
