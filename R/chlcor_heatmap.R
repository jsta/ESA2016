...
gg <- ggplot(data = dt, aes(Var2, Var1, fill = value)) + geom_raster() + viridis::scale_fill_viridis() + geom_text(aes(fill = dt$value, label = dt$value)) + coord_flip() + theme_minimal() + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), legend.position = "none") + ggtitle(expression(paste("Correlation Matrix (",rho, ")")))
