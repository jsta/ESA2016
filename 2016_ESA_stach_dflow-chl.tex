...
\subtitle{\nohyphens{ }}
\date{August 2016}
\author{{\Medium Jemma Stachelek}, Christopher Madden, Stephen P. Kelly, Michelle Blaha}
...
\begin{frame}{Quantifying phytoplankton abundance}

	\begin{centering}
	\begin{tikzpicture}
		\node(img1)[anchor=south west,inner sep=0] at (0,0) {\includegraphics[height=6cm,keepaspectratio=true]{images/landsat_border.jpg}};
		\node(img2) at (8.7,1.2){\includegraphics[height=2cm,keepaspectratio=true]{images/fl-inset_border.png}};
	\end{tikzpicture}
...
\begin{frame}{The Discrete Approach}
	\begin{columns}
		\begin{column}[c]{5.5cm}
			...
			\includegraphics[height=4.7cm,clip=true,trim = 0mm 0mm 0mm 0mm,keepaspectratio=true]{figures/fbmap_wqmn_red.png}%
			\end{center}
		\end{column}
...
\begin{frame}{Bay-Wide Network}
			\begin{columns}
				...
				\begin{centering}
					\begin{tikzpicture}
						\node[anchor=south west,inner sep=0] at (0,0) {\includegraphics[width=5.9cm,clip=true,trim = 0mm 40mm 0mm 0mm,keepaspectratio=true]{figures/chltimeseries.png}};
						% \draw[help lines,xstep=1,ystep=1](0,0) grid (5.9,7.4);
						% \foreach \x in {0,1,...,6}{ \node [anchor=north] at (\x,0) {\x};}
						% \foreach \y in {0,1,...,7}{ \node [anchor=east] at (0,\y) {\y};}
					 	% \draw[red,thick, rounded corners](3.3,5.8) rectangle (4.1,6.3);
					 	\draw[red,thick, rounded corners](1.9,0.7) rectangle (2.4,2.3);
					 	\draw[red,thick, rounded corners](3.6,0.7) rectangle (4.6,2.3);
					\end{tikzpicture}
					\end{centering}
				\end{column}
			\end{columns}
...
\begin{frame}{Inverse Path Distance Weighting (IPDW)}
	\phantom{As the crow flies or as the fish swims?\footnote{Little et al. 1997}\footnote[frame]{Suominen et al. 2010}}
	%\vspace{2pt}
 	\begin{columns}
   	\begin{column}{6.8cm}
